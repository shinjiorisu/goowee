...
We are going to see what the _User Experience_ of a Goowee application looks like on different devices.
...
To layout the fields in a Goowee `Form` we use the `cols` parameter. It represents the number of columns the field will occupy in a line. Since Goowee uses https://getbootstrap.com/[Bootstrap,window=_blank] under the hood, we use its https://getbootstrap.com/docs/5.3/layout/grid/[Grid System,window=_blank] to decide how to layout fields in a form.
...
As we have seen, Goowee applications work out of the box on different devices. They are not optimized for any one of them, but hey, they work without you having to worry about it.
