...
Goowee applications can be configured from the _Tenant Properties_ _(System Administration -> Settings)_. That means each tenant can have a different setup.
...
Each Goowee application can be configured with three different colors:
...
Since we don’t want to manually set the colors every time we install the application, we can use the `onTenantInstall` method to set the _Tenant Properties_ like the follow.
...
18:47:36.982 INFO  [restartedMain] d.elements.core.ApplicationService       : --------------------------------------------------------------------------------
18:47:36.987 INFO  [restartedMain] d.elements.core.ApplicationService       : Executing 'goowee.core.beforeInit'...
18:47:37.029 INFO  [restartedMain] d.elements.core.ApplicationService       : Executing 'com.example.init'...
18:47:37.030 INFO  [restartedMain] d.elements.core.ApplicationService       : Executing 'goowee.core.afterInit'...
...
In the next chapter we are going to see what happens when we use a Goowee application from a Desktop Computer, from a Tablet and from a Mobile Phone.
