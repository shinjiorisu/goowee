...
I love _Philosophy_. I could write thousands of words about Goowee but there’s another thing I love: *Practice*.

So let’s write our first Goowee application, shall we?

NOTE: If you are a seasoned developer, or you’re in a hurry, install https://www.oracle.com/java/technologies/downloads/?er=221886#java{version-java}[Java {version-java}, window=_blank], go download the https://github.com/goowee-projects/goowee-app-template[Goowee Template Application, window=_blank], run it with `./gradlew bootRun` and these are your three steps.

=== Apache Grails

Goowee is built on top of the Grails Framework. The Grails website gives us what we need to quickly start up with a _“blank”_ application project.
...
...
    ...
    implementation 'org.goowee:goowee-core:{revnumber}'
}
----
...
We can now play around with the basics of a Goowee application. When done, we can terminate the application pressing `CTRL + C`.
