...
=== Goals
*With _Goowee_ you can develop back-office web applications writing code in a single programming language: https://groovy-lang.org/[Apache Groovy,window=_blank]. No need to know HTML, CSS o Javascript.*

With _Goowee_ you can develop and maintain web applications with requirements such as:
...
NOTE: The main purpose of _Goowee_ is to decouple the business logic from the _User Interface (GUI)_. This lowers the costs of maintaining the application: just one language instead of 5 (HTML, CSS, JavaScript, Java, SQL), less skilled people can join the team and no need to upgrade the GUI when new web technologies/standards gets available.

=== Non-Goals
*_Goowee_ is NOT a solution for:*
...
_Goowee_ applications are Grails applications so you need to get confident with the https://grails.org/[Apache Grails,window=_blank] framework and the https://groovy-lang.org/[Apache Groovy,window=_blank] programming language.
...
== Basics
In _Goowee_ everything is a component. All visual objects of the application are derived from the same base class `Component` and they can be assembled together like we do with a LEGO set.
...
=== The Shell
The `Shell` component is the _Goowee_ GUI. Each _Goowee_ application share a common user experience and content structure.
...
==== Responsiveness
All _Goowee_ applications work both on desktop computers and on mobile devices by design and without the developer having to cope with it. Here is how an application looks like on a Desktop, on a Tablet and on a Mobile Phone.
...
=== Project Structure
_Goowee_ applications are Grails applications. The project structure, follows the _conventions over configuration_ design paradigm so each folder contains a specific source file type.
...
A _Goowee_ application is a container for a set of _Features_.
...
Each application has a `DEFAULT` database connection defined in the `grails-app/conf/application.yml` file. This `DEFAULT` connection cannot be changed at runtime and it is used by _Goowee_ to store its own database.
...
. You can edit/create database connections at runtime from the _Goowee_ GUI accessing with the `super` user from the menu `System Configuration -> Connection Sources`
...
New Tenants can be created from the _Goowee_ GUI accessing as `super` from the menu `System Configuration -> Tenants`. If multi-tenancy is not a requirement to your application you will be using the `DEFAULT` Tenant which is automatically created.

=== User Management

Users can access a _Goowee_ application with credentials made of a username and a secret password. Each user must be configured by the Tenant's `admin` user from the menu `Tenant Administration -> Users` and `Tenant Administration -> Groups`.
...
<2> Implementing `ElementsController` the _Goowee_ API will become available
